...
"""
Canges:
In Functions evaluate_line and read_file:
...
def evaluate_line(line):
    ...
    # e.g. M30 GCodeStringlen=3 / GCodeStringlen-1 = 2 (0..2)
    GCodeString = line
    ...
    250 Used for inching
    251 Used for measure tool length
    252 Used to detect last step in steptransfer to teensy
    253 M98 Subprogram call (M98 298 -> 253 Subprogram call)
    254 M99 Subprogram end (M99 299 -> 254 Subprogram end)
    255 O   Subprogram number
    """
    ...
    while GcodeIndex <= GCodeStringlen:
        ...
        if Letter == "G" or Letter == "g":
            ...
            # G52 or G54 or G74 or G68
            if GCode == 52 or GCode == 54 or GCode == 74 or GCode == 68:
                Value3 = 0 # Reserve
                Value4 = 0 # Reserve

            # G90 or G91 or G69
            if GCode == 90 or GCode == 91 or GCode == 69:
                Value0 = 0 # Reserve
                Value1 = 0 # Reserve
                Value2 = 0 # Reserve
                Value3 = 0 # Reserve
        ...
        if Letter == "M" or Letter == "m":
            ...
            if ValueString.isnumeric:
                ...
                #M98 298 -> 253 Subprogram call
                #M99 299 -> 254 Subprogram end
                if GCode == 298:
                    GCode = 253
                if GCode == 299:
                    GCode = 254                

            # M00 or M01 or M02 or M05 or M08 or M09 or M10 or M11 or M35 or M36 or M30 or M99(254)
            if GCode == 200 or GCode == 201 or GCode == 202 or GCode == 205 \
            or GCode == 208 or GCode == 209 or GCode == 210 or GCode == 211 \
            or GCode == 235 or GCode == 236 or GCode == 230 or GCode ==254:
                Value0 = 0 # Reserve
                Value1 = 0 # Reserve
                Value2 = 0 # Reserve
                Value3 = 0 # Reserve
        ...
        if Letter == "O" or Letter == "o":
            ...
            # Transfer values
            if ValueString.isnumeric:
                Value0 = int(ValueString) *10000 # Subprogram number
                Value1 = 0 # Reserve
                Value2 = 0 # Reserve
                Value3 = 0 # Reserve
                Value4 = 0 # Reserve

        """ Key letter X, Y, Z, I, J, F, S, P, R, L found """
        ...
        or Letter == "R" or Letter == "r" \
        or Letter == "L" or Letter == "l":
            # Initialize values
            LetterFound = True
            ValueString = ""
            ValueNegative = False
            ...
            if Letter == "S" or Letter == "s": # M03, M04, S = Speed spindle
                Value0 = TmpValue       
            if Letter == "P" or Letter == "p": # G04 P = Dwell time / M98 P = Subprogram number
                Value0 = TmpValue
            if Letter == "R" or Letter == "r": # G68 R = Rotation angle
                Value2 = TmpValue                
            if Letter == "L" or Letter == "l": # M98 L = Number of repeats
                Value1 = TmpValue
...
def read_file(name):
    ...
    with open(name, 'r') as f:
        ...
        for line in f:
            ...
            if Valid == True:
                # Copy G-Code to array, if Amount of lines < 7498
                # Array in Teensy is 7500 big, one line is used to detect last step
                if i < 7498:
                    Move[i].GCode = GCode
    ...
    # Insert last line, to detect transfer end
    Move[i].GCode = 252 # 252 Used to detect last step in steptransfer to teensy
    Move[i].Value0 = 0
    Move[i].Value1 = 0
    Move[i].Value2 = 0
    Move[i].Value3 = 0
    Move[i].Value4 = 0
    Move[i].NameStep = "Last step"
    Move[i].ActualLineInFile = j-1
    ReturnValue += 1
    # Test Print G-Comand
    #print(i)
    #print("Stepdata GCode %d Value0 %d Value1 %d Value2 %d Value3 %d Value4 %d" %(Move[i].GCode, Move[i].Value0, Move[i].Value1, Move[i].Value2, Move[i].Value3, Move[i].Value4))
